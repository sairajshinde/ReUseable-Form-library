...
loadData() {
 ...
 this.api.getPendingRequest(this.currentYear, this.selectedYear, this.selectedMonth).subscribe({
      next: (res) => {
...
mapApplicationStatus(code: string): string {
  if (['L', 'LC'].includes(code)) return 'New Application';
...
private openRemarkModal(): Promise<string | null> {
  return new Promise(resolve => {
    ...
    // Always clear remarks when modal is about to show
    modalElement.addEventListener(
      'show.bs.modal',
      () => {
        this.remarks = '';  // 🔑 resets input value each time
      },
      { once: true } // attach only for this open cycle
    );

    // Clear again on hide (just in case)
    modalElement.addEventListener(
      'hidden.bs.modal',
      () => {
        this.remarks = '';
...
onRemarkClose(submit: boolean) {
  if (this.modalResolve) {
    if (submit) {
      // User clicked submit → pass whatever is typed (even empty string)
      this.modalResolve(this.remarks);
    } else {
      // User closed/cancelled → return null
