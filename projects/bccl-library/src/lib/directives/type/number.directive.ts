...
export class NumberDirective {
  ...
  onKeyDown(event: KeyboardEvent) {
    ...
    if (isDot) {
      // Only allow dot if decimals are allowed (afterLimit > 0)
      if (afterLimit === 0 || hasDot || currentValue.length === 0) {
        event.preventDefault();
      }
      return;
    }
  ...
  onPaste(event: ClipboardEvent) {
    ...
    const [before, after] = pastedText.split('.');
    if (
      (before?.length || 0) > beforeLimit ||
      (afterLimit === 0 && after) || // Prevent pasting decimals if not allowed
      (after?.length || 0) > afterLimit
    ) {
      event.preventDefault();
    }
  }
